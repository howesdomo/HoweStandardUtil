        ...
        , Util.XamariN.IBluetooth
    {
        ...
        public void OnServiceConnected(ProfileType profile, IBluetoothProfile proxy)
        {
            ...
            foreach (BluetoothDevice item in proxy.ConnectedDevices)
            {
                ...
                var args = new Util.XamariN.BLuetoothEventArgs(new BluetoothDeviceInfo(item.Name, item.Address, (int)item.Type, (int)item.BondState) { IsConnected = true });
